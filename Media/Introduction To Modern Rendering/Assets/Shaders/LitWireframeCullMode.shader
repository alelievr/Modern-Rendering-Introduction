...
Shader "Lit/WireframeCullMode"
{
    Properties
    {
        ...
        _BaseColorFront ("Base Front Color", Color) = (0.0, 0.0, 0.0, 1.0)
        _BaseColorBack ("Base Back Color", Color) = (0.0, 0.0, 0.0, 1.0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            uniform float4 _BaseColorFront;
            uniform float4 _BaseColorBack;
            ...
            fixed4 frag(g2f i, uint face : SV_isFrontFace) : SV_Target
            {
                float minDistanceToEdge = min(i.dist[0], min(i.dist[1], i.dist[2])) * i.dist[3];
                i.normal = normalize(i.normal);

                float4 color = face > 0 ? _BaseColorFront : _BaseColorBack;
                ...
                fixed4 finalColor = lerp(color, _WireColor, t);

                half3 worldNormal = UnityObjectToWorldNormal(i.normal);
                half nl = max(0, dot(worldNormal, _WorldSpaceLightPos0.xyz));
                finalColor *= nl * _LightColor0;
