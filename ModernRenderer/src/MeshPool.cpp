...
std::vector<uint32_t> MeshPool::meshletVertices;
...
{
    unsigned poolIndex = meshCount;

	meshCount++;

	// TODO: rework all the meshlet indices + triangle indices to make it work

    // Append vertices to the pool, storing the index offset to update meshlet data
    int vertexOffset = vertices.size();
	vertices.insert(vertices.end(), mesh->vertices.begin(), mesh->vertices.end());

	meshlets.insert(meshlets.end(), mesh->meshlets.begin(), mesh->meshlets.end());
	meshletVertices.insert(meshletVertices.end(), mesh->meshletVertices.begin(), mesh->meshletVertices.end());
...
void MeshPool::AllocateMeshPoolBuffers(std::shared_ptr<Device> device)
{
	...
	RenderUtils::AllocateVertexBufer(device, meshletVertices, ViewType::kStructuredBuffer, gli::FORMAT_UNDEFINED, "Meshlet Vertices", meshletIndicesPool, meshletIndicesPoolView);
